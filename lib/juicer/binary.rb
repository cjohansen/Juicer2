...
module Juicer
  ...
  module Binary
    ...
    #
    # [<tt>options</tt>]
    #     Hash of options, optional. Defines which options are allowed on the
    #     module along with their default value.
    #
    ...
    def execute(params = nil)
      Open3.popen3("#{self.command} #{params}") do |stdin, stdout, stderr|
        begin
          if block_given?
            yield stdin, stdout
          else
          ...
          raise RuntimeError, "Unable to execute #@binary"
        end
      end
    ...
    # 
    # [<tt>opt</tt>]
    #     The option to return value for
    #
    def get_opt(opt)
      @options[opt] || nil
    end
    ...
    #
    # Returns all available options as an array of symbols
    #
    def available_options
      @options.keys.map { |option| option.to_sym }
    end
